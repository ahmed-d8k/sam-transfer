...
class SAMTransferDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Method 2
        # embed_mask = base_mask.copy()
        # embed_mask = embed_mask.convert('RGB')
        # embed_mask = np.array(embed_mask)
        # embed_mask = cv2.resize(embed_mask, (1024, 1024), interpolation = cv2.INTER_AREA)
        # embed_mask = torch.from_numpy(embed_mask).to(torch.uint8)
        # embed_mask = self.reshape(embed_mask)
...
optimizer = torch.optim.Adam(sam_model.mask_decoder.parameters(), lr=1e-4, weight_decay=0) #1e-5 default
